...
class AccountantController extends Controller
{
    ...
    public function updateStatus(Request $request, StockRequisition $requisition)
    {
        ...
        // Log activity
        ActivityLog::record('updated_status', auth()->user()->name . ' marked request ' . $requisition->reference_number . ' as ' . $request->status, 'StockRequisition', $requisition->id);
    ...
    public function uploadPayment(Request $request, StockRequisition $requisition)
    {
        ...
        // Log activity
        ActivityLog::record('uploaded_payment', auth()->user()->name . ' uploaded payment of $' . $request->amount . ' for ' . $requisition->reference_number, 'Payment', $requisition->id);

        // Notify employee
        Notification::send(
            $requisition->user_id,
            'Payment Receipt Uploaded',
            'Payment receipt for ' . $requisition->reference_number . ' has been uploaded. Amount: $' . $request->amount,
            'success',
            route('employee.requisitions.show', $requisition)
        );
